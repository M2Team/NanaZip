...
static LPCTSTR const kExtractLocation = TEXT("Open Destinationn After Extraction");
...
static bool ReadFMOption(LPCTSTR value)
{
  ...
  if (key.Open(HKEY_CURRENT_USER, kCU_FMPath, KEY_READ) == ERROR_SUCCESS)
  {
      if (key.QueryValue(value, enabled) == ERROR_SUCCESS)
      {
          wprintf(L"Read setting: %s = %s\n", value, enabled ? L"true" : L"false");
          return enabled;
      }
      key.Close();
  }
  wprintf(L"Failed to read setting: %s\n", value);
  return enabled;
}
...
{
  OutputDebugString(L"Saving settings...\n");
  ...
  SaveOption(kExtractLocation, ExtractLocation);
  // SaveOption(kUnderline, Underline);

  SaveOption(kShowSystemMenu, ShowSystemMenu);
  OutputDebugString(L"Settings saved.\n");
}

void CFmSettings::Load()
{
  OutputDebugString(L"Loading settings...\n");
  ShowDots = false;
  ...
  if (key.Open(HKEY_CURRENT_USER, kCU_FMPath, KEY_READ) == ERROR_SUCCESS)
  {
    ...
    ReadOption(key, kExtractLocation, ExtractLocation);
    // ReadOption(key, kUnderline, Underline);

    ReadOption(key, kShowSystemMenu, ShowSystemMenu );
    OutputDebugString(L"Settings loaded.\n");
  }
}
...
bool ExtractLocation() { return ReadFMOption(kExtractLocation); } //added
