...
static constexpr size_t GUID_STRING_LENGTH = ARRAYSIZE(L"{12345678-90AB-CDEF-1234-567890ABCDEF}");
...
struct DECLSPEC_UUID("{542CE69A-6EA7-4D77-9B8F-8F56CEA2BF16}") CNanaZipCopyHookFactory
    : winrt::implements<CNanaZipCopyHookFactory, IClassFactory> {
    ...
    try {
        *ppv = NULL;
        if (rclsid == __uuidof(CNanaZipCopyHookFactory))
            return winrt::make<CNanaZipCopyHookFactory>()->QueryInterface(riid, ppv);
        return E_INVALIDARG;
    } catch (...) {
        return winrt::to_hresult();
    }
...
_Use_decl_annotations_ STDAPI DllRegisterServer() {
    try {
        ...
        auto clsidLen = StringFromGUID2(__uuidof(CNanaZipCopyHookFactory), clsid, ARRAYSIZE(clsid));
        if (!clsidLen)
        ...
        auto dllPathLen = GetModuleFileNameW(g_hInstance, dllPath, ARRAYSIZE(dllPath));
...
_Use_decl_annotations_ STDAPI DllUnregisterServer() {
    try {
        ...
        if (!StringFromGUID2(__uuidof(CNanaZipCopyHookFactory), clsid, ARRAYSIZE(clsid)))
            winrt::throw_hresult(E_OUTOFMEMORY);
